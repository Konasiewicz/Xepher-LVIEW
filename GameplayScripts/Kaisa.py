...
def Combo(game):
    ...
    if use_w_in_combo and IsReady(game, w_spell):
        ...
        if target and ValidTarget(target) and not IsCollisioned(game, target):
            w_spell.move_and_trigger(game.world_to_screen(castpoint_for_collision(game, w_spell, game.player, target)))
...
def Laneclear(game):
    ...
    if lane_clear_with_w and IsReady(game, w_spell):
        ...
        if minion and ValidTarget(minion): #and not IsCollisioned(game, minion)
            w_spell.move_and_trigger(game.world_to_screen(castpoint_for_collision(game, w_spell, game.player, minion)))
